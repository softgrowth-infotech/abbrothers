...
if(isset($_POST['submit']))
{
	...
	$date1="menu_".date('dmYhis').".$extension1";
	if(empty($errors1))
	{
...
try{
	...
	if($row_check)
	{
		...
		$stmt = $conn->prepare("INSERT INTO `menu_details`(`category_id`,`menu_name`,`prize`,`image`,`status`)VALUES(:category_id,:menu_name,:prize,:image,:status)");

		$executed=$stmt->execute(array(':image' => $date1,':category_id' =>$category_id,':menu_name' =>$menu_name ,':prize' =>$prize ,':status' => $status));
			   if($executed)
			   {
				echo "<script>alert('Added Successfully....');window.location.href='menu_list';</script>";
			   }
	}catch(Exception $e) {
                  ...
                  <form id="demo-form2" data-parsley-validate class="form-horizontal form-label-left" method="post" action="" enctype="multipart/form-data">
                   ...
                   <div class="form-group">
                       ...
                       <select name="category_id"  class="form-control col-md-7 col-xs-12" required="required">
                        ...
                        $row=$stmt->fetchAll(PDO::FETCH_ASSOC);
                          for($i=0;$i<count($row);$i++)
                            {
                        ?>
                         <option value="<?php echo $row[$i]['menu_id']; ?>"><?php echo $row[$i]['category_name']; ?></option>
                       <?php } ?>
                       </select>
                      </div>
                    </div>
                     ...
                     <div class="form-group">
                      ...
                      <div class="col-md-6 col-sm-6 col-xs-12">
                        <input type="text" name="menu_name"  class="form-control col-md-7 col-xs-12" placeholder="Enter Menu Name" required="required">
                      </div>
                    </div>
                    ...
                    <div class="form-group">
                      ...
                      <div class="col-md-6 col-sm-6 col-xs-12">
                        <input id="" class="form-control col-md-7 col-xs-12" type="text" name="prize" required="required" placeholder="Enter Menu Prize">
                      </div>
                    </div>
